...
<#assign
<#-- Print only steps with these types of execution messages. Must be "PASS" "ERROR" "INFORMATION" or "WARNING" -->
STEPS_ENABLED = "ERROR"
...
STEP_FILTER="DeclareExecutionIssue"
...
TEST_ASSET_LOCATION="Test Asset Location"
...
EXTRACTEDDATA_COLUMN_NAMES=["Time", "Test Case", "Procedure", "Step", "ID", "Extracted Data Tag", "Value(s)"]
	...
	<style type="text/css">
		...
		*, *:before, *:after {
		...
		.b-table-cell.b-table-cell_header {
			...
			overflow: hidden;
			/*white-space: nowrap; ITEST-14429: prevent word overriding*/
			margin-top: 1px;
		}
...
<#macro renderExtractedData>
	...
	<@renderTableFrame title=EXTRACTEDDATA_HEADER colNames=EXTRACTEDDATA_COLUMN_NAMES 
		colGroups=["g-col-index", "g-col-short-name", "g-col-short-name", "g-col-step", "g-col-step", "g-col-label", "g-col-label"]>
		<#list model.report.extractedData as extractedData>
		<tr class="b-table-row b-table-row_colored">
			<td class="b-table__cell">
				<div class="b-table-cell">
					<div class="b-table-cell__item">
						<div class="b-text">${extractedData.time}</div>
					</div>
				</div>
			</td>
			<td class="b-table__cell">
				<div class="b-table-cell">
					<div class="b-table-cell__item">
						<div class="b-text">${extractedData.testCase}</div>
					</div>
				</div>
			</td>
			<td class="b-table__cell">
				<div class="b-table-cell">
					<div class="b-table-cell__item">
						<div class="b-text">${extractedData.procedure}</div>
					</div>
				</div>
			</td>
			<td class="b-table__cell">
				<div class="b-table-cell">
					<div class="b-table-cell__item">
						<div class="b-text">${extractedData.executedStep}</div>
					</div>
				</div>
			</td>
			<td class="b-table__cell">
				<div class="b-table-cell">
					<div class="b-table-cell__item">
						<div class="b-text">${extractedData.executableStep}</div>
					</div>
				</div>
			</td>
			<td class="b-table__cell">
...
<#macro renderStepsTable>
	...
	<#list model.issues as issue>
		<#if issue.stepId?has_content && issue.severity = STEPS_ENABLED>
			<#assign stepList = stepList + [issue.stepId]>
		</#if>
	</#list>
	<@renderTableFrame title=STEP_HEADER colNames=STEP_COLUMN_NAMES colGroups=["g-col-index", "g-col-step-action", "g-col-step-start-time", "g-col-step-duration"]>
		<#list model.steps as step>
			<#if step.stepId?has_content && stepList?seq_contains(step.stepId)>
			...
			<tr class="b-table-row b-table-row_colored" style="${getIssueHighlighting(issue)}">
				<td class="b-table__cell">
					<#if issue.stepId?has_content>
						<div class="b-table-cell">
							<div class="b-table-cell__item">
								<a class="b-link" href="#step_${issue.stepId}">${issue.stepId}</a>
							</div>
						</div>
					</#if>
				</td>
				...
				<div class="b-topology-preview">
					<#if isVelocityTopology>
						...
						<div class="b-topology-preview__link">
							<a draggable="false" href="${getTopologyQuery(true)}" target="_parent"
							   class="b-link b-link_full-size"></a>
						</div>
					</#if>
					<#if isItestTopology>
						<div class="b-topology-preview__preview">
